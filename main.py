...
class AppToDo:
    ...
    def adicionar_tarefa(self, e):
        ...
        if self.entrada_tarefa.value:
            try:
                ...
                self.atualizar_lista_tarefas()
            except Exception as ex: # Captura erro de tarefa duplicada (UNIQUE)
                self.page.snack_bar = ft.SnackBar(
                    content=ft.Text(f"Erro: A tarefa '{self.entrada_tarefa.value}' já existe.", color=self.cor['texto']),
                    bgcolor=self.cor['destaque']
                )
    ...
    def excluir_tarefa(self, tarefa):
        ...
        self.page.snack_bar.open = True
        self.atualizar_lista_tarefas() # Atualiza a lista após a exclusão
